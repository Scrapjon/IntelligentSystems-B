...
if __name__ != "__main__":
    from ImageRecognition.models import NeuralNetwork, MLPnn
else:
    from models import NeuralNetwork, MLPnn
...
class ImageRecognizer():
    def __init__(self, batch_size=64, model_path=None, model_type=ModelType.CNN):
        ...
        if self.model_type == ModelType.CNN:
            ...
            self.__initialize_MLP__(model_path=model_path)
            self.loss_fn = nn.CrossEntropyLoss()
            self.optimizer = torch.optim.Adam(self.model.parameters(), lr=1e-3)
            print("Initialised MLP")
        else:
            raise ValueError("Invalid model_type specified. Choose 'nn' or 'svc'.")
        
    #initialise MLP
    def __initialize_MLP__(self, model_path=None):
        makedirs(MODEL_FOLDER, exist_ok=True)
        self.__initialize_dataloaders__(self.batch_size)
